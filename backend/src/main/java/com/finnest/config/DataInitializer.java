...
public class DataInitializer {
    ...
    public CommandLineRunner loadTestData(
        ...
        return args -> {
            ...
            cleanTenantData(oldDummyTenantId, goalRepo, projectRepo, expenseRepo, bankAccountRepo, chitFundRepo,
                    peerLendingRepo);

            // No automatic data seeding for users to keep accounts blank

            // 2. Clean up any previously seeded default dummy data for all existing users
            List<User> users = userRepo.findAll();
            for (User user : users) {
                cleanDefaultSeededData(user.getTenantId(), goalRepo, projectRepo, expenseRepo, bankAccountRepo, chitFundRepo,
                        peerLendingRepo);
            }
        };
    }
    ...
    private static void cleanDefaultSeededData(
            UUID tenantId,
            GoalRepository goalRepo,
            ProjectRepository projectRepo,
            ExpenseRepository expenseRepo,
        ...
        TenantContext.setCurrentTenant(tenantId.toString());
        try {
            goalRepo.findAllByTenantIdOrderByTargetDateAsc(tenantId).stream()
                    .filter(g -> "Car Insurance 2024".equals(g.getName()) || 
                                 "Home Renovation".equals(g.getName()) || 
                                 "New Phone".equals(g.getName()))
                    .forEach(goalRepo::delete);

            projectRepo.findAllByTenantIdOrderByCreatedAtDesc(tenantId).stream()
                    .filter(p -> "New House Construction".equals(p.getName()) || 
                                 "Wedding Planning".equals(p.getName()))
                    .forEach(projectRepo::delete);

            expenseRepo.findAllByTenantIdOrderByExpenseDateDesc(tenantId).stream()
                    .filter(e -> "Weekly grocery shopping".equals(e.getDescription()) || 
                                 "Electricity bill".equals(e.getDescription()))
                    .forEach(expenseRepo::delete);

            chitFundRepo.findAllByTenantIdOrderByCreatedAtDesc(tenantId).stream()
                    .filter(c -> "Vinayaka Chits".equals(c.getOrganizerName()))
                    .forEach(chitFundRepo::delete);

            peerLendingRepo.findAllByTenantIdOrderByCreatedAtDesc(tenantId).stream()
                    .filter(pl -> "Ramesh Kumar".equals(pl.getPersonName()))
                    .forEach(peerLendingRepo::delete);

            bankAccountRepo.findAllByTenantIdOrderByCreatedAtDesc(tenantId).stream()
                    .filter(b -> "SBIN00012345".equals(b.getAccountNumber()) || 
                                 "HDFC987654321".equals(b.getAccountNumber()))
                    .forEach(bankAccountRepo::delete);
        } finally {
            TenantContext.clear();
        }
    }
