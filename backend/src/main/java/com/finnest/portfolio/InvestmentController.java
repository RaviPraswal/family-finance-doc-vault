...
public class InvestmentController {
    ...
    @PostMapping
    public ResponseEntity<Investment> create(@RequestBody Investment entity) {
        entity.setTenantId(UUID.fromString(TenantContext.getCurrentTenant()));
        return ResponseEntity.ok(repository.save(entity));
    }

    @PutMapping("/{id}")
    public ResponseEntity<Investment> update(@PathVariable UUID id, @RequestBody Investment entity) {
