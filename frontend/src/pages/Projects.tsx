...
export default function Projects() {
  ...
  const [formData, setFormData] = useState({ name: '', budget: '', status: 'IN_PROGRESS', startDate: '' });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setFormData({ name: '', budget: '', status: 'IN_PROGRESS', startDate: '' });
      fetchProjects();
    } catch (err) {
      console.error(err);
    }
    ...
    <div className="h-full flex flex-col">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {projects.map((project) => (
          <div key={project.id} className="bg-card p-6 rounded-lg shadow-sm border border-border">
            <div className="flex justify-between items-start mb-4">
              ...
              <span className={`px-2 py-1 text-xs font-medium rounded-full ${project.status === 'COMPLETED' ? 'bg-green-100 text-green-700' : 'bg-yellow-100 text-yellow-700'}`}>
